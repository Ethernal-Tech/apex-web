...
const getCardanoLatestSlot = async (chain: ChainEnum): Promise<bigint> => {
	const url = getAppConfig().cardanoApiUrl;
	const apiKey = process.env.CARDANO_API_API_KEY || 'test_api_key';
	if (!url) {
		throw new InternalServerErrorException('cardano api url not configured');
	}
...
export const getLatestBlockOrSlot = async (
	...
	try {
		...
		const url = getAppConfig().nexusRpcUrl;
		if (!url) {
			throw new InternalServerErrorException(
				'RPC URL not configured. Set NEXUS_RPC_URL.',
			);
		}
