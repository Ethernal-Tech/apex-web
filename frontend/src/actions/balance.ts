...
export const getWalletBalanceAction = async (
	chain: ChainEnum,
	address: string,
) => {
...
export const fetchAndUpdateBalanceAction = async (dispatch: Dispatch) => {
	...
	const { account, networkId } = accountInfo;
	...
	const balanceResp = await tryCatchJsonByAction(() =>
		getWalletBalanceAction(chain, account),
	);
