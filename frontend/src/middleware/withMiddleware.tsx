  ...
  const WithMiddlewareComponent: React.FC<P> = (props) => {
    ...
    if (isMobileDevice) {
      ...
      <Box 
        ...
        <Typography sx={{mt:4}}>It looks like you're accessing the {bridgeVersion} Bridge on a mobile device. {bridgeVersion} Bridge is currently only available as a web application and requires a desktop browser with the <Link href="https://chromewebstore.google.com/detail/eternl/kmhcihpebfmpgmihbkipmjlmmioameka" target="_blank" rel="noreferrer">Eternl</Link> chrome extension.</Typography>
        
        <Typography sx={{mt:2}}>To use the {bridgeVersion} Bridge, please follow these steps:</Typography>
        ...
        <List component={'ol'} sx={{listStyle: "decimal", paddingLeft:3}}>
          ...
          <ListItem sx={{ display: "list-item" }}>Connect via the Eternl Wallet extension.</ListItem>
          <ListItem sx={{ display: "list-item" }}>{appSettings.isSkyline ? 'Move your AP3X or ADA Tokens' : 'Move your AP3X Tokens'}</ListItem>
        </List>
