...
const HomePage: React.FC = () => {
	...
	const { chain: srcChain, destinationChain: dstChain } = useSelector(
		...
		<BasePage>
			<Typography
				...
				sx={{
					...
					lineHeight: '',
					fontSize: '44px',
					fontFamily: appSettings.isSkyline
						? 'Goldman, sans-serif'
						: 'Major Mono Display, sans-serif',
				}}
			>
			...
			<img
				src={appSettings.isSkyline ? SkylineBridgeGraph : BridgeGraph}
				alt="apex bridge graph"
				style={{ height: '280px', marginTop: '32px' }}
			/>
			...
			>
				<Box>
					...
					<CustomSelect
						...
						value={
							srcChainOptions.some((x) => x.value === srcChain)
								? srcChain
								: ''
						}
				...
				<Box>
					...
					<CustomSelect
						...
						value={
							dstChainOptions.some((x) => x.value === dstChain)
								? dstChain
								: ''
						}
						disabled={
							(isLoggedInMemo && !appSettings.isMainnet) ||
							dstChainOptions.length < 2
						}
