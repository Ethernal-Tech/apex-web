...
const BridgeInput = ({
	...
	const minUtxoValueDfm = useSelector(
		(state: RootState) => state.settings.minUtxoValue,
	);
	...
	const maxAmountDfm: string = totalDfmBalance
		? chain === ChainEnum.Prime || chain === ChainEnum.Vector
			...
			: (
					...
					BigInt(bridgeTxFee) -
					BigInt(minDfmValue)
				).toString() // for nexus, using minDfm value as substitute to user wallet fee / potential fee
		: '0';
