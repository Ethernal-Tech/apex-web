...
const PasteApexAmountInput: React.FC<PasteApexAmountInputProps> = ({
	sx,
	maxAmounts,
	text,
	setAmount,
	...
	const isMaxAmountEntered =
		maxSendableDfm &&
		BigInt(maxSendableDfm) > 0 &&
		convertApexToDfm(text, chain) !== maxSendableDfm.toString();

	// Returns true if entered value to send exceedes the maximum amount a user can send (balance - fees)
	const hasInsufficientBalance =
		BigInt(convertApexToDfm(text, chain)) > maxAmounts.maxByBalance;
