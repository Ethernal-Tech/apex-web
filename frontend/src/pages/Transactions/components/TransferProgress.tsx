...
interface TransferProgressProps {
	...
	setTx: React.Dispatch<
		React.SetStateAction<BridgeTransactionDto | undefined>
	>;
}

const TransferProgress = ({ tx, setTx }: TransferProgressProps) => {
	...
	const transferProgress = (function (txStatus: TransactionStatusEnum) {
		switch (txStatus) {
			case TransactionStatusEnum.ExecutedOnDestination:
				return TRANSFER_PROGRESS_TEXT.DONE;
			case TransactionStatusEnum.InvalidRequest:
				return TRANSFER_PROGRESS_TEXT.ERROR;
			default:
				return TRANSFER_PROGRESS_TEXT.IN_PROGRESS;
		}
	})(txStatusToShow);
	...
	return (
		<Box>
			...
			>
				{steps.map((step) => (
					<TransferStep key={step.number} step={step} />
				))}
			</Box>
