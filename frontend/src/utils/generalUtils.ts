...
export const validateSubmitTxInputs = (
	...
	if (isCardanoChain(sourceChain)) {
		...
		if (
			BigInt(amount) < BigInt(convertDfmToWei(settings.minValueToBridge))
		) {
			return `Amount too low. The minimum amount is ${convertUtxoDfmToApex(settings.minValueToBridge)} ${tokenInfo.label}`;
		}
...
export const formatTxDetailUrl = (
	transaction: BridgeTransactionDto,
): string => {
	...
	for (const utxo of utxos) {
		for (const asset of utxo.output.amount) {
			if (asset.unit === 'lovelace') {
				...
				value.set_coin(
					existing.checked_add(BigNum.from_str(asset.quantity)),
				);
			...
			const policyScriptHash = ScriptHash.from_bytes(
				Buffer.from(policyId, 'hex') as unknown as Uint8Array,
			);
			const assetName = AssetName.new(
				Buffer.from(assetNameHex, 'hex') as unknown as Uint8Array,
			);
