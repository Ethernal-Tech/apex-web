...
const convertBits = (
    ...
    if (pad) {
        ...
        if (bits >= inBits) captureAndThrowError('Excess padding', "utils.ts", "convertBits");
        if ((value << (outBits - bits)) & maxV) captureAndThrowError('Non-zero padding', "utils.ts", "convertBits");
    }
