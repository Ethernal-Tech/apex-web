...
export class LockedTokensControllerClient extends BaseClient {
     ...
     * @return FOR OPENAPI SCHEMA GENERATION ONLY: Contains enum definitions.
     */
    getHealth(): Promise<TokenEnumSchemaHolderDto> {
        let url_ = this.baseUrl + "/lockedTokens/health";
        url_ = url_.replace(/[?&]$/, "");
    ...
    protected processGetHealth(response: Response): Promise<TokenEnumSchemaHolderDto> {
        const status = response.status;
        let _headers: any = {}; if (response.headers && response.headers.forEach) { response.headers.forEach((v: any, k: any) => _headers[k] = v); };
        if (status === 299) {
            return response.text().then((_responseText) => {
            let result299: any = null;
            let resultData299 = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
            result299 = TokenEnumSchemaHolderDto.fromJS(resultData299);
            return result299;
            });
        } else {
            return response.text().then((_responseText) => {
            ...
            let resultDatadefault = _responseText === "" ? null : JSON.parse(_responseText, this.jsonParseReviver);
                resultdefault = resultDatadefault !== undefined ? resultDatadefault : null as any;
    
            return throwException("Returns a simple health status message.", status, _responseText, _headers, resultdefault);
            });
        }
    }
}
...
export class CreateCardanoTransactionResponseDto implements ICreateCardanoTransactionResponseDto {
    ...
    nativeTokenAmount?: string | undefined;
...
export class TokenEnumSchemaHolderDto implements ITokenEnumSchemaHolderDto {
    token!: TokenEnum;

    [key: string]: any;

    constructor(data?: ITokenEnumSchemaHolderDto) {
        if (data) {
            for (var property in data) {
                if (data.hasOwnProperty(property))
                    (this as any)[property] = (data as any)[property];
    ...
    static fromJS(data: any): TokenEnumSchemaHolderDto {
        data = typeof data === 'object' ? data : {};
        let result = new TokenEnumSchemaHolderDto();
        result.init(data);
        return result;
    }
...
export interface ITokenEnumSchemaHolderDto {
    token: TokenEnum;
