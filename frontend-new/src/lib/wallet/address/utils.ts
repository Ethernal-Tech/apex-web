...
import { captureAndThrowError, captureException } from "../errors";
...
export const IsAddressWithValidPrefix = (addr: string): boolean => {
  return addr.startsWith("addr") || addr.startsWith("stake");
};
...
export const Bech32EncodeFromBase256 = (
  hrp: string,
  data: Uint8Array,
): string | undefined => {
  try {
...
export const Bech32DecodeToBase256 = (
  raw: string,
): Bech32DecodeData | undefined => {
  try {
