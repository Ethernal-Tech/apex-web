...
import { longRetryOptions, retry, retryForever } from "@/lib/wallet/utils";
...
export const signAndSubmitCardanoTx = async (
  ...
  if (txResult.status === "rejected") {
    console.error("Cardano wallet submitTx failed:", txResult.reason);
...
export const signAndSubmitEthTx = async (
  ...
  const onTxHash = (txHash: any) => {
    ...
    submitActionPromise = tryCatchJsonByAction(bindedSubmittedAction, false);
  };
