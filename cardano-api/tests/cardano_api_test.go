...
import (
	...
	"github.com/Ethernal-Tech/cardano-api/api/model/common/request"
...
func TestCardanoAPI(t *testing.T) {
	...
	bytesContent, err := embededConfigFiles.ReadFile(configFileName)
	require.NoError(t, err)
	...
	dstChainConfig := config.CardanoChains["cardano"]
...
func waitForTxBridging(
	...
	}, infracommon.WithRetryCount(80), infracommon.WithRetryWaitTime(time.Second*15))
